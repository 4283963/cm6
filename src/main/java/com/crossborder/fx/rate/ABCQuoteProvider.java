...
@Component
public class ABCQuoteProvider implements BankQuoteProvider {

    private static final Logger log = LoggerFactory.getLogger(ABCQuoteProvider.class);
    ...
    private static final BigDecimal BUY_OFFSET = new BigDecimal("-0.0002");
    private static final BigDecimal SELL_OFFSET = new BigDecimal("0.0004");
    private static final BigDecimal FLUCTUATION_RANGE = new BigDecimal("0.002");
    ...
    public BankQuote fetchQuote(Currency baseCurrency, Currency quoteCurrency) {
        ...
        try {
            ...
            if (ThreadLocalRandom.current().nextDouble() < FAILURE_RATE) {
                throw new RuntimeException("ABC 询价请求失败：连接被拒绝");
            }
