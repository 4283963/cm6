...
@Component
public class BOCQuoteProvider implements BankQuoteProvider {

    private static final Logger log = LoggerFactory.getLogger(BOCQuoteProvider.class);
    ...
    private static final BigDecimal BUY_OFFSET = new BigDecimal("0.0001");
    private static final BigDecimal SELL_OFFSET = new BigDecimal("-0.0001");
    private static final BigDecimal FLUCTUATION_RANGE = new BigDecimal("0.001");

    private static final int MIN_DELAY_MS = 30;
    ...
    public BankQuote fetchQuote(Currency baseCurrency, Currency quoteCurrency) {
        ...
        try {
            ...
            if (ThreadLocalRandom.current().nextDouble() < FAILURE_RATE) {
                throw new RuntimeException("BOC 询价请求失败：服务暂时不可用");
            }
